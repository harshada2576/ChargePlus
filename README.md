...
The **authoritative schema is `supabase/migrations/*.sql`** (Phase 1 Steps 1.3/1.4/1.5/1.6/1.7 authored and executed the `public`, `analytics`, and `ml` schemas — 29 tables — with Step 1.6 constraints/indexes and Step 1.7 RLS & security policies EXECUTED + VERIFIED on the linked Supabase project). Apply them to a linked Supabase project:
...
Step 1.6 constraints & indexes synthesis and Step 1.7 RLS & security policies migrations are EXECUTED + VERIFIED against the linked Supabase project (29 active target tables with RLS enabled, 29 public RLS policies, 0 analytics/ml client policies, defense-in-depth table/column grants, 0 cross-layer FKs, 9 frozen legacy tables untouched). Full end-to-end clean flow verification is Phase 1 Step 1.10.
